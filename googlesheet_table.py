...
class GoogleTable:
    ...
    def search_abonement(
        self,
        abonement_number: str,
        search_col: int = 1,
        balance_col: int = 4,
        end_date_col: int = 5,
    ) -> Union[int, List[Union[str, int]]]:
        googlesheet_client: pygsheets.client.Client = self._get_googlesheet_client()
        wks = self._get_googlesheet_file_url(googlesheet_client)
        
        try:
            find_cell = wks.find(abonement_number, matchEntireCell=True, cols=(search_col, search_col))[0]
        except IndexError:
            return -1
